...
while i < number_pizzas #In large loop, we select each individual pizza.
  puts "Pizza No. #{i + 1}:"
  puts "Would you like a regular or specialty pizza?"
  puts "Enter 1 for regular, 2 for specialty."
  reg_spe = gets.chomp.to_i
  ...
  puts "What size would you like, 10, 14 or 18 inches?"
    ...
    puts "All our specialty pizzas are 18 inch"
    puts "Enter 1 or Everything, 2 for All-Meat,"
    puts "3 for All-Veggies, 4 for Thai Shrimp"
    specialty = gets.chomp.to_i
    if specialty == 1
      puts "Everything pizza"
      total_price = total_price + 19.99
    elsif specialty == 2
      puts "All-Meat pizza"
      total_price = total_price + 19.99
    elsif specialty == 3
      puts "All-Veggies pizza"
      total_price = total_price + 19.99
    elsif specialty == 4
      puts "Thai Shrimp pizza"
      total_price = total_price + 23.99
    else
      puts "Wrong number, start over."
      exit (0)
    end
  ...
  end ###closes long reg_spe if statement
  i += 1
